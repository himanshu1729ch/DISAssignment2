...
namespace DIS_Assignment2__Fall_2021
{
    class Program
    {
        ...
        */
        ...
        public static int LargestAltitude(int[] gain)
        {
            try
            {
                int maximum = 0;
                int currentAlti = 0;
                if ((gain.Length >= 1 && gain.Length <= 100) && (gain[i] >= -100 && gain[i] <= 100))  //performimg check on constraints
                {
                    for (int i = 0; i < gain.Length; i++)   //for loops starts 
                    {
                        currentAlti =currentAlti + gain[i];
                        maximum = Math.Max(currentAlti, maximum); //comparing current altitude and maximum reached using math.max function
                    }     //for loops ends
                }
                return maximum;    //returning the maximum altitude
            }
            catch (Exception)
            {
        ...
        */
        ...
        public static int SearchInsert(int[] nums, int target)
        {
            try
            {
                ...
                int begin = 0, end = nums.Length - 1;  // declaring the begin and end
                while (begin <= end)   //starting the while loop
                {
                    ...
                    if (nums[(begin + end) / 2] > target)     //now using binary search to find the appropriate index
                        end = (begin + end) / 2;              //it targets for mid values 
        ...
        */
        ...
        public static List<string> CommonChars(string[] words)
        {
            try
            {
                ...
                for (int i = 0; i < words.Length; i++)  //for loops begins
                {
                    
                    countArray.Add(new int[26]);  //appending the array with int sized array for each word

                    
                    foreach (char c in words[i].ToCharArray())   //for each loop to count character in collection
                    {
                        int index = c - 'a';
                        countArray[i][index]++;
                    }
        ...
        public static bool UniqueOccurrences(int[] arr)
        {
            try
            {
                ...
                {
                    if (numberOccurences.ContainsKey(arr[i]))    //checking the key in dictionary if it contains then enter the statements
                    {
                ...
                int a = numberOccurences.Values.Distinct().Count();   //Counting Disticnt values from the dictionary 
        ...
        */
        ...
        public static int CountMatches(List<List<string>> items, string ruleKey, string ruleValue)
        {
            try
            {
                ...
                foreach (var item in items)    // checking item in items
                {
        ...
        */
        ...
        public static int CountConsistentStrings(string allowed, string[] words)
        {
            try
            {
                ...
                var dict = new Dictionary<char, bool>();  // creating dictionary with character key and boolean value
        ...
        */
        ...
        public static int[] AnagramMappings(int[] nums1, int[] nums2)
        {
            try
            {
                ...
                for (int i = 0; i < nums1.Length; i++)  //for loop begins
                {
